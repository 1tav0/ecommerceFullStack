...
const productImgResize = async (req, res, next) => {
  ...
  await Promise.all( // Uses Promise.all to concurrently process multiple files in parallel. 
    ...
    req.files.map(async (file) => { // Iterates over each uploaded file in req.files.
      ...
      await sharp(file.path) // Creates a Sharp instance for the specified file.
        ...
        .toFile(`public/images/products/${file.filename}`); // Saves the processed image to the specified directory with the original filename.
    })
  );
  next();
}
